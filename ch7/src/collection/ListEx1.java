 ...
 *  Map
 *  
 *  List 인터페이스 : 순서가 존재함, 데이터 중복 허용       (배열과 비슷하고 배열의 개선된 형태)
...
public class ListEx1 {
	public static void main(String[] args) {
		...
		// add(int index, 요소)
		list.add(3, 6); 
		System.out.println("특정 위치에 추가 후 "+list); // [1, 2, 3, 6, 4, 5] 3번자리에 변경이 아닌 6을 추가
		
		// set(int index, 변경할 요소)
		list.set(3, 7);
		System.out.println("특정 위치에 요소 변경 "+list); // [1, 2, 3, 7, 4, 5] 3번자리에 6을 7로 변경
		
		// remove(int index)
		String result=list2.remove(2);
		System.out.println(result);
		System.out.println("특정 위치 요소 제거 후 "+list2); // [자바, html, 서블릿]
		
		// remove(Object o)
		list4.remove("바나나");
		System.out.println("특정 위치 요소 제거 후 "+list4); // [사과, 포도, 메론, 딸기, 참외]
		...
		Collections.sort(list4); // 오름차순으로 정렬
		System.out.println("정렬 후 "+ list4); // [딸기, 메론, 사과, 참외, 포도]
		
		// Comparator.naturalOrder() : 오름차순, Comparator.reverseOrder() : 내림차순
		list.sort(Comparator.reverseOrder());
		System.out.println("정렬 후 "+ list); // [7, 5, 4, 3, 2, 1]
