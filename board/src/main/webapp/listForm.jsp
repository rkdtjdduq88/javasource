...
<form class="row g-3 justify-content-center" action='<c:url value="/list.do"/>'>
	<div class="col-auto">
		<select class="form-select" name="criteria">
			...
			<option value="title" <c:out value="${pageDTO.criteria == 'title'?'selected':''}"/>>제목</option>
			<option value="name" <c:out value="${pageDTO.criteria == 'name'?'selected':''}"/>>작성자</option>
			<option value="content" <c:out value="${pageDTO.criteria == 'content'?'selected':''}"/>>내용</option>
		</select>
	</div>
	<div class="col-md-5">
